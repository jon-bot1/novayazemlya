...
// Swedish Mining Village — NOW SPLIT INTO TWO SEPARATE MAPS
// Surface map: village with guard booths, cabins, machinery, crew building (2000x1800)
// Underground map: mine tunnels, ore carts, boss arena (2000x1000)
// Player transitions via mine elevator interaction

const SURFACE_W = 2000;
const SURFACE_H = 1800;
const MINE_W = 2000;
const MINE_H = 1000;

// Y offset: underground coords were y 1800-2800, now remapped to 0-1000
const MINE_Y_OFFSET = 1800;
...
// ══════════════════════════════════════════════════════════
// SURFACE MAP (village, cabins, machine hall, mine entrance)
// ══════════════════════════════════════════════════════════
export function generateMiningSurfaceMap() {
  const T = 10;
  enemyId = 30000;
  containerId = 30000;

  const terrainZones: TerrainZone[] = [
    { x: 0, y: 0, w: SURFACE_W, h: 200, type: 'forest' },
    { x: 0, y: 0, w: 150, h: SURFACE_H, type: 'forest' },
    { x: SURFACE_W - 150, y: 0, w: 150, h: SURFACE_H, type: 'forest' },
    { x: 150, y: 200, w: SURFACE_W - 300, h: 1600, type: 'dirt' },
  ...
  const crewWalls: Wall[] = [
    ...
    makeWall(crewX + crewW - T, crewY + 160, T, crewH - 160, CONCRETE),
    makeWall(crewX + 170, crewY + T, T, 100, CONCRETE),
    makeWall(crewX + 170, crewY + 180, T, crewH - 180 - T, CONCRETE),
  ];

  // --- SMALL CABINS ---
  const cabinSize = 100;
  const cabinPositions = [
    { x: 200, y: 700 }, { x: 200, y: 880 },
    { x: 350, y: 700 }, { x: 350, y: 880 },
  ];
  const cabinWalls: Wall[] = cabinPositions.flatMap(c => [
    ...
    makeWall(c.x + cabinSize - T, c.y + 90, T, 10, WOOD),
  ]);

  // --- GUARD BOOTHS ---
  const boothSize = 60;
  const boothPositions = [
    { x: 850, y: 250 }, { x: 850, y: 1500 }, { x: 1500, y: 600 },
  ];
  ...
  const machineHallWalls: Wall[] = [
    ...
    makeWall(machX + machW - T, machY + 180, T, machH - 180, CONCRETE),
  ...
  const shedWalls: Wall[] = [
    ...
    makeWall(shedX + shedW - T, shedY + 100, T, 20, WOOD),
  ];

  const walls: Wall[] = [...crewWalls, ...cabinWalls, ...boothWalls, ...machineHallWalls, ...shedWalls];
  ...
  const ZONE_FOREST_N = { x: 30, y: 30, w: SURFACE_W - 60, h: 170 };
  ...
  // Svarta Solen operative — rare spawn
  if (Math.random() < 0.25) {
  ...
  const props: Prop[] = [
    ...
    // East forest
    ...Array.from({ length: 10 }, (_, i) => ({
      pos: { x: SURFACE_W - 30 - Math.random() * 110, y: 250 + i * 160 + Math.random() * 50 },
      w: 26 + Math.random() * 14, h: 26 + Math.random() * 14,
      type: 'pine_tree' as Prop['type'],
    })),
    // Guard booths
  ...
  // Extraction points — surface only
  const allExfils: ExtractionPoint[] = [
    { pos: { x: 100, y: 100 }, radius: 80, timer: 5, active: false, name: 'FOREST ROAD NORTH' },
    { pos: { x: SURFACE_W - 100, y: 900 }, radius: 80, timer: 5, active: false, name: 'EAST LOGGING TRAIL' },
  ...
  // Elevator — transitions to underground
  const elevatorDown: ExtractionPoint = { pos: { x: 950, y: 1750 }, radius: 50, timer: 2, active: true, name: '⛏ MINE ELEVATOR — DESCEND ⛏' };
  (elevatorDown as any)._isElevator = true;
  (elevatorDown as any)._elevatorDirection = 'down';
  allExfils.push(elevatorDown);
  ...
  const lights: LightSource[] = [
    ...
    { pos: { x: SURFACE_W - 100, y: 900 }, radius: 80, color: '#44ff66', intensity: 0.4, type: 'fire' },
  ...
  return { walls, enemies, lootContainers, documentPickups, extractionPoints: allExfils, alarmPanels, props, lights, windows, terrainZones, mapWidth: SURFACE_W, mapHeight: SURFACE_H };
}

// ══════════════════════════════════════════════════════════
// UNDERGROUND MAP (mine tunnels, boss arena)
// All Y coordinates shifted by -1800 so map starts at y=0
// ══════════════════════════════════════════════════════════
export function generateMineUndergroundMap() {
  const T = 10;
  // Use different ID ranges to avoid conflicts
  enemyId = 31000;
  containerId = 31000;

  const O = MINE_Y_OFFSET; // subtract this from original y coords

  const terrainZones: TerrainZone[] = [
    { x: 0, y: 0, w: MINE_W, h: MINE_H, type: 'concrete' },
    { x: 200, y: 100, w: 1600, h: 800, type: 'concrete' },
  ];

  const mineWalls: Wall[] = [
    // Elevator shaft walls
    makeWall(850, 0, T, 200, MINE_WALL),
    makeWall(1050, 0, T, 200, MINE_WALL),
    // Main tunnel north wall
    makeWall(200, 100, 650, T, MINE_ROCK),
    makeWall(1050, 100, 750, T, MINE_ROCK),
    // Main tunnel south wall
    makeWall(200, 300, 1600, T, MINE_ROCK),
    // West tunnel branch
    makeWall(200, 100, T, 500, MINE_ROCK),
    makeWall(400, 100, T, 200, MINE_ROCK),
    makeWall(400, 300, T, 300, MINE_ROCK),
    makeWall(200, 600, 200, T, MINE_ROCK),
    // East tunnel branch
    makeWall(1600, 100, T, 500, MINE_ROCK),
    makeWall(1800, 100, T, 500, MINE_ROCK),
    makeWall(1600, 600, 200, T, MINE_ROCK),
    // Boss arena (center)
    makeWall(600, 400, T, 400, MINE_ROCK),
    makeWall(1400, 400, T, 400, MINE_ROCK),
    makeWall(600, 800, 800, T, MINE_ROCK),
    // Small side room
    makeWall(250, 300, 150, T, MINE_ROCK),
    makeWall(250, 300, T, 120, MINE_ROCK),
    makeWall(250, 420, 150, T, MINE_ROCK),
  ];

  const walls = mineWalls;

  // Zones (remapped from original)
  const ZONE_MINE_SHAFT = { x: 860, y: 10, w: 180, h: 180 };
  const ZONE_MINE_MAIN_TUNNEL = { x: 410, y: 110, w: 630, h: 180 };
  const ZONE_MINE_EAST_TUNNEL = { x: 1060, y: 110, w: 530, h: 180 };
  const ZONE_MINE_WEST_BRANCH = { x: 210, y: 110, w: 180, h: 480 };
  const ZONE_MINE_EAST_BRANCH = { x: 1610, y: 110, w: 180, h: 480 };
  const ZONE_BOSS_ARENA = { x: 610, y: 410, w: 780, h: 380 };

  const PLAYER_SPAWN = { x: 950, y: 50 }; // elevator exit
  ...
  const enemies: Enemy[] = [
    ...
    // BOSS: GRUVRÅ
    (() => {
      const bp = randIn(ZONE_BOSS_ARENA.x, ZONE_BOSS_ARENA.y, ZONE_BOSS_ARENA.w, ZONE_BOSS_ARENA.h);
  ...
  const lootContainers: LootContainer[] = [
    ...
    makeLoot(1000, 600, 'weapon_cabinet', 'weapon_cabinet_swedish'),
    {
      id: `loot_${containerId++}`,
  ...
  const docZones = [ZONE_MINE_MAIN_TUNNEL, ZONE_MINE_WEST_BRANCH, ZONE_BOSS_ARENA];
  const docIds = ['doc_mine_3', 'doc_mine_4', 'doc_mine_5'];
  const documentPickups: DocumentPickup[] = docIds.map((id, i) => {
    const z = docZones[i % docZones.length];
    const p = randIn(z.x, z.y, z.w, z.h);
    return makeDocPickup(p.x, p.y, id);
  ...
  const props: Prop[] = [
    { pos: { x: 500, y: 150 }, w: 30, h: 30, type: 'wood_crate' },
    { pos: { x: 700, y: 200 }, w: 22, h: 22, type: 'barrel_stack' },
    { pos: { x: 1200, y: 160 }, w: 40, h: 20, type: 'equipment_table' },
    { pos: { x: 1400, y: 200 }, w: 30, h: 30, type: 'wood_crate' },
    { pos: { x: 300, y: 400 }, w: 22, h: 22, type: 'barrel_stack' },
    { pos: { x: 1700, y: 400 }, w: 30, h: 30, type: 'wood_crate' },
    // Boss arena pillars
    { pos: { x: 800, y: 550 }, w: 30, h: 30, type: 'concrete_barrier' },
    { pos: { x: 1000, y: 500 }, w: 30, h: 30, type: 'concrete_barrier' },
    { pos: { x: 1200, y: 600 }, w: 30, h: 30, type: 'concrete_barrier' },
    { pos: { x: 900, y: 700 }, w: 30, h: 30, type: 'concrete_barrier' },
    { pos: { x: 1100, y: 700 }, w: 30, h: 30, type: 'concrete_barrier' },
  ];

  const alarmPanels: AlarmPanel[] = [
    { id: 'alarm_underground', pos: { x: 900, y: 150 }, activated: false, hacked: false, hackProgress: 0, hackTime: 5 },
  ];

  // Extraction points
  const allExfils: ExtractionPoint[] = [];

  // Deep shaft exit (boss must be dead)
  const bossExfil: ExtractionPoint = { pos: { x: 1000, y: 780 }, radius: 60, timer: 3, active: true, name: 'DEEP SHAFT EXIT' };
  (bossExfil as any)._requirements = 'boss_dead';
  allExfils.push(bossExfil);

  // Elevator back up
  const elevatorUp: ExtractionPoint = { pos: { x: 950, y: 50 }, radius: 50, timer: 2, active: true, name: '⛏ MINE ELEVATOR — ASCEND ⛏' };
  (elevatorUp as any)._isElevator = true;
  (elevatorUp as any)._elevatorDirection = 'up';
  allExfils.push(elevatorUp);

  const lights: LightSource[] = [
    { pos: { x: 950, y: 100 }, radius: 100, color: '#ffaa44', intensity: 0.4, flicker: true, type: 'ceiling' },
    { pos: { x: 500, y: 180 }, radius: 80, color: '#ffaa44', intensity: 0.3, flicker: true, type: 'ceiling' },
    { pos: { x: 1300, y: 180 }, radius: 80, color: '#ffaa44', intensity: 0.3, flicker: true, type: 'ceiling' },
    { pos: { x: 300, y: 350 }, radius: 70, color: '#ff8833', intensity: 0.3, flicker: true, type: 'fire' },
    { pos: { x: 1700, y: 350 }, radius: 70, color: '#ff8833', intensity: 0.3, flicker: true, type: 'fire' },
    // Boss arena — eerie red glow
    { pos: { x: 1000, y: 600 }, radius: 200, color: '#ff4422', intensity: 0.4, flicker: true, type: 'alarm' },
    { pos: { x: 800, y: 550 }, radius: 100, color: '#ff6633', intensity: 0.3, flicker: true, type: 'fire' },
    { pos: { x: 1200, y: 650 }, radius: 100, color: '#ff6633', intensity: 0.3, flicker: true, type: 'fire' },
  ];

  const windows: WindowDef[] = [];

  return { walls, enemies, lootContainers, documentPickups, extractionPoints: allExfils, alarmPanels, props, lights, windows, terrainZones, mapWidth: MINE_W, mapHeight: MINE_H };
}

// Legacy wrapper — returns surface map by default (engine handles underground separately)
export function generateMiningVillageMap() {
  return generateMiningSurfaceMap();
}
